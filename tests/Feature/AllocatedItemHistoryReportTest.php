...
class AllocatedItemHistoryReportTest extends TestCase
{
    ...
    public function test_allocated_item_history_displays_stock_transfers_dispatched_and_downloaded()
    {
        $role = Role::firstOrCreate(['name' => 'Admin', 'slug' => 'admin']);
        $role->permissions()->sync(Permission::all());
        $user = User::factory()->create(['role_id' => $role->id]);
        ...
        \App\Models\StockMovement::create([
            ...
            'movement_type' => \App\Models\StockMovement::TYPE_TRANSFER,
            'transaction_type' => 1,
            'quantity' => -1.00,
            'unit_cost' => 100,
            ...
            'remarks' => 'Stock Transfer Out #ST000001 (Item: TESTITEM002)',
            'created_by' => $user->id,
        ]);
        ...
        \App\Models\StockMovement::create([
            ...
            'movement_date' => now()->toDateString(),
            'business_date' => now()->toDateString(),
            'remarks' => 'Stock Transfer In #ST000001 (Item: TESTITEM002)',
            'created_by' => $user->id,
        ]);
        ...
        $response->assertStatus(200);
        $response->assertSee('Stock Transfer Out (Dispatched)');
        $response->assertSee('Stock Transfer In (Downloaded)');
    }
}
