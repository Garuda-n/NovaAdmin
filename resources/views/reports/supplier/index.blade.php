<x-app-layout>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h2 class="font-bold text-xl text-slate-800 dark:text-slate-100 leading-tight flex items-center gap-2">
                    ...
                    Supplier Procurement & Analysis Report
                </h2>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    Review supplier-wise purchase activities, inward stock bills, total quantity procured, and vendor balances.
                </p>
            </div>
            <div class="flex items-center gap-2">
                <button type="button" onclick="printReportContent('report-table-container', 'Supplier Procurement & Analysis Report')" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition">
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6" x-data="reportFilters({
            ...
            filterUrl: '{{ route('reports.supplier.search') }}',
            ...
            <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm space-y-4">
                <form @submit.prevent="applyFilter()" class="space-y-4">
                    ...
                    <!-- Row 2: Supplier Specific Filters -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3.5 pt-2 border-t border-slate-100 dark:border-slate-700/60">
                        <!-- Search Input -->
                        <div>
                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-1">Search Supplier</label>
                            <input type="text" name="search_text" placeholder="Name, Code, Mobile, GST..." class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white text-xs py-2 focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-1">Supplier Type</label>
                            <select name="supplier_type" class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white text-xs py-2 focus:ring-indigo-500 focus:border-indigo-500">
            ...
            <div id="report-table-container">
                @include('reports.supplier._table', ['reportData' => $reportData])
            </div>

            <!-- Supplier Inwards History Modal Pop-up -->
            <div id="supplier-inwards-modal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                <div class="relative w-full max-w-4xl bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden transform transition-all my-8">
                    ...
                    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
                        <h3 class="text-base font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                            ...
                            Supplier Procurement & Inward History
                        </h3>
                        <button type="button" onclick="closeSupplierInwardsModal()" class="text-slate-400 hover:text-slate-600 dark:hover:text-white p-1 rounded-lg transition">
                            <x-heroicon-o-x-mark class="w-6 h-6" />
                        </button>
                    </div>

                    <!-- Modal Body Container -->
                    <div id="supplier-inwards-modal-body" class="p-6">
                        <div class="flex items-center justify-center py-12 text-slate-400">
                            ...
                            <span>Loading supplier stock inward history...</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <script>
                function openSupplierInwardsModal(supplierId) {
                    const modal = document.getElementById('supplier-inwards-modal');
                    const modalBody = document.getElementById('supplier-inwards-modal-body');
                    ...
                    fetch('/reports/supplier/' + supplierId + '/inwards-modal', {
                        headers: {
                    ...
                    .then(data => {
                        if (data.success && data.html) {
                            ...
                            modalBody.innerHTML = '<p class="text-center text-rose-500 py-8">Failed to load supplier stock inward history.</p>';
                        }
                    })
                    .catch(err => {
                        console.error(err);
                        modalBody.innerHTML = '<p class="text-center text-rose-500 py-8">An error occurred while fetching supplier history.</p>';
                    });
                }

                function closeSupplierInwardsModal() {
                    const modal = document.getElementById('supplier-inwards-modal');
                    const modalBody = document.getElementById('supplier-inwards-modal-body');
