        ...
        <div class="overflow-hidden rounded-xl border border-slate-700 bg-slate-900 shadow-lg">
            <table class="min-w-full border-collapse">
                <thead class="bg-slate-800">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-gray-300">
                            #
                        </th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-gray-300">
                            Counter Name
                        ...
                        <th class="px-6 py-4 text-center text-xs font-semibold uppercase text-gray-300">
                            Status
                        </th>
                ...
                <tbody>
                    @forelse($counters as $counter)
                        <tr class="border-b border-slate-700 transition hover:bg-slate-800 last:border-b-0">
    ...
    <div
        ...
        <div class="relative flex min-h-screen items-center justify-center p-6">
            <div class="w-full max-w-2xl rounded-xl border border-slate-700 bg-slate-900 shadow-2xl">
                <!-- Modal Header -->
                <div class="flex items-center justify-between border-b border-slate-700 px-6 pt-5 pb-3">
                    ...
                    <button
                        id="closeModal"
                        type="button"
                        class="text-gray-400 transition hover:text-white text-lg">
                        ✕
                    </button>
                </div>
                <!-- Modal Body -->
                <form id="branchForm" method="POST">
                    ...
                    <div class="px-6 pt-4">
                        ...
                        <div class="pb-3 mb-1">
                            <p class="text-sm text-gray-400">
                                Counter : <span id="counterTitle" class="font-semibold text-emerald-400"></span>
                            </p>
                        </div>
                        <hr class="border-slate-700">
                        <!-- Branch List -->
                        <div class="max-h-80 overflow-y-auto divide-y divide-slate-700">
                            @forelse($branches as $branch)
                                <label
                                    class="flex cursor-pointer items-center gap-3 px-1 py-3 transition hover:bg-slate-800">
                                    <input
                                        type="checkbox"
                                        name="branch_ids[]"
                                        value="{{ $branch->id }}"
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-700 px-6 py-4">
                        <button
                            type="button"
                            id="cancelModal"
                            class="rounded-lg bg-slate-700 px-5 py-2 text-sm font-medium text-white transition hover:bg-slate-600">
                            Cancel
                        </button>
                        <button
                            type="submit"
    ...
    <script>
        ...
        document.querySelectorAll('.assignBranchBtn').forEach(button => {
            button.addEventListener('click', function () {
                ...
                form.action = `/counters/${id}/branch-mapping`;
