    ...
    <div class="flex items-center justify-between pb-4 border-b border-slate-700/60">
        <div>
            ...
            <p class="text-slate-400 text-xs mt-1">
                ...
                | Type: <span class="text-slate-200 font-semibold">{{ $supplier->supplier_type }}</span>
                @if($supplier->supplier_code)
                    | Code: <span class="font-mono text-slate-300 font-semibold">{{ $supplier->supplier_code }}</span>
                @endif
                | Created via: <span class="text-slate-300 font-semibold">{{ $supplier->created_through }}</span>
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-[#1c2538] rounded-xl p-5 border border-[#27334d] space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4 text-xs">
                <div>
                    <span class="text-slate-400 block mb-1">Supplier Type</span>
                    <span class="text-slate-200 font-semibold text-sm">{{ $supplier->supplier_type }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">Contact Person</span>
                    <span class="text-slate-200 font-semibold text-sm">{{ $supplier->contact_person ?: '-' }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">Mobile Number</span>
                    <span class="text-slate-200 font-semibold text-sm">{{ $supplier->mobile }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">Alternate Mobile</span>
                    <span class="text-slate-200">{{ $supplier->alternate_mobile ?: '-' }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">Email Address</span>
                    <span class="text-slate-200">{{ $supplier->email ?: '-' }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">GST Number</span>
                    <span class="text-slate-200 font-mono font-semibold">{{ $supplier->gst_number ?: 'N/A' }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">PAN Number</span>
                    <span class="text-slate-200 font-mono font-semibold">{{ $supplier->pan_number ?: 'N/A' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-[#1c2538] rounded-xl p-5 border border-[#27334d] space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4 text-xs">
                <div>
                    ...
                    <span class="text-slate-200">{{ $supplier->country->name ?? '-' }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">State</span>
                    <span class="text-slate-200">{{ $supplier->state->name ?? '-' }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">City</span>
                    <span class="text-slate-200">{{ $supplier->city->name ?? '-' }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">Pincode</span>
                    <span class="text-slate-200 font-mono">{{ $supplier->pincode }}</span>
                </div>

                <div class="col-span-2">
                    <span class="text-slate-400 block mb-1">Branch</span>
                    <span class="text-slate-200 font-semibold">{{ $supplier->branch ? $supplier->branch->name : 'Global (All Branches)' }}</span>
                </div>

                <div class="col-span-2">
                    <span class="text-slate-400 block mb-1">Full Address</span>
                    <span class="text-slate-200">{{ $supplier->address ?: '-' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="md:col-span-2 bg-[#1c2538] rounded-xl p-5 border border-[#27334d] space-y-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-5 gap-4 text-xs">
                <div>
                    <span class="text-slate-400 block mb-1">Opening Balance</span>
                    <span class="text-indigo-400 font-bold text-sm">₹{{ number_format($supplier->opening_balance, 2) }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">Credit Limit</span>
                    <span class="text-indigo-400 font-bold text-sm">₹{{ number_format($supplier->credit_limit, 2) }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">Credit Days</span>
                    <span class="text-slate-200 font-bold text-sm">{{ $supplier->credit_days }} Days</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">Created By</span>
                    <span class="text-slate-200">{{ $supplier->creator->name ?? 'System' }}</span>
                </div>

                <div>
                    <span class="text-slate-400 block mb-1">Last Updated By</span>
                    <span class="text-slate-200">{{ $supplier->updater->name ?? 'System' }}</span>
                </div>
            </div>
        </div>
